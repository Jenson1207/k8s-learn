...
redis_stop() {
    ...
    # if tls enabled then get tls-port otherwise get port
    ...
    if am_i_root; then
        ...
        # /opt/bitnami/redis/bin/redis-cli -a pass -p port shutdown
...
redis_configure_default() {
    ...
    # /opt/bitnami/redis/tmp/redis.pid
    rm -f "$REDIS_BASE_DIR/tmp/redis.pid"

    # ${REDIS_BASE_DIR}" "${REDIS_DATA_DIR}" "${REDIS_BASE_DIR}/tmp" "${REDIS_LOG_DIR} 
    # keep above those dir exist and chown redis:redis
    redis_configure_permissions
